...
import experiments.{ActiveExperiments, DotcomponentsRendering, Participant}
...
object RenderingTierPicker {
  ...
  def getTier(page: PageWithStoryPackage)(implicit request: RequestHeader): RenderType = {
    ...
    val isParticipant = ActiveExperiments.groupFor(DotcomponentsRendering) == Participant
    ...
    else if (request.isGuui || (isParticipant && isSupported && isWhitelisted)) RemoteRender
    else LocalRender
  }
}
